...
def get_sis(root, address):
    # return sister for binary branching
    left = get_left_sis(root, address)
    right = get_right_sis(root, address)
    if left:
        return left[0].address
    if right:
        return right[0].address
    else:
        return None
